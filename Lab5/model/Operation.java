...
public class Operation {
    ...
    public static Polynomial simpleThreaded(Polynomial p1, Polynomial p2) throws
        ...
        int step = result.getCoefficients().size() / NR_THREADS;
        if (step == 0) {
            step = 1;
        }

        int end;
        for (int i = 0; i < result.getCoefficients().size(); i += step) {
            end = i + step;
    ...
    public static Polynomial karatsubaSequential(Polynomial p1, Polynomial p2) {
        ...
        Polynomial r2 = Polynomial.addZeros(Polynomial.substract(Polynomial.substract(z2, z3), z1), len);

...
namespace lab4.impl
{
    class TaskImpl
    {
        ...
        private static void StartClient(string host, int id)
        {
            ...
            var ipAddr = ipHostInfo.AddressList[0]; // separate ip of host
            var remEndPoint = new IPEndPoint(ipAddr, Parser.PORT); // create endpoint

            var client = new Socket(ipAddr.AddressFamily, SocketType.Stream, ProtocolType.Tcp); // create client socket
